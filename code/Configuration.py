class Configuration:
	...
	def __init__(self):
		...
		self.costPedestrian = 0.1	# cost of pedestrian c_s
		self.costPassengers = 0.1 	# cost of passengers c_t
	...
	def set_best_candidates(self):
		self.BEST_CANDIDATES_COUNT = int(self.POPULATION_SIZE * self.best_ratio)  # Number of Best Candidates to Use
		#self.set_path()
	
	def set_random_candidates(self):
		self.RANDOM_CANDIDATES_COUNT = int(self.POPULATION_SIZE * self.random_ratio)  # Number of Random Candidates (From Entire Population of Generation) to Next Population
		#self.set_path()
