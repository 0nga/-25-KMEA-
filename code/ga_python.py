...
POPULATION_SIZE = 1000  # Max Number of Individuals in Each Population
...
ALTRUISM = 0.2		# altruistic behavior
...
path="/Users/aloreggia/Downloads/test/1000ge/pythonTest_altruism_"+str(ALTRUISM)+"_probPed_"+str(probDeathPedestrians)
...
class Individual:
	...
	def computeFitness(self, scenario, avgKnobLevel=0):
		...
		utility_straight_max_pass=((numberOfPassengers+1) * selfish - self.altruism   - costPedestrian * dead )
		utility_straight_min_pass = (-(numberOfPedestrians+1) * self.altruism   - (numberOfPedestrians+1) * costPedestrian * dead )
		
		utility_turn_max_pass = (self.altruism  - (numberOfPassengers+1) * selfish)
		utility_turn_min_pass = ((numberOfPedestrians+1) * self.altruism)
...
def save_options(path=path):
	...
	f.write("numberOfPedestrians: "+ str(numberOfPedestrians+1) + "\n")
	f.write("numberOfPassengers: "+ str(numberOfPassengers+1) + "\n")
...
def main(ALTRUISM, path):
	...
	for current_generation in range(MAX_GENERATIONS):
		...
		for p in population:
			...
			nPass=random.randint(0,numberOfPedestrians+1)
