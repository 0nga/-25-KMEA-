...
def main():
	...
	for current_generation in range(conf.MAX_GENERATIONS):
		...
		# Aggiorna la fitness con il self-esteem
		for i, individual_p in enumerate(population):
			if df_scenari_input_for_nn.shape[0] > i:
				...
				# Calcolo dell'utilità per la svolta (1) e per andare dritto (0)
				# Queste formule dovrebbero riflettere la stessa logica utilitaristica che la NN sta cercando di apprendere
				# quando si calcola la fitness.

				# Utilità dell'ANDARE DRITTO (0)
				# Esempio: Vantaggio passeggeri, costo pedoni
				util_dritto = (s_numPass * (1 - s_probPass) * s_selfish_scenario) - \
				              (s_numPed * s_probPed * s_altruism_scenario)


				# Utilità della SVOLTA (1)
				# Esempio: Vantaggio pedoni, costo passeggeri
				util_svolta = (s_numPed * (1 - s_probPed) * s_altruism_scenario) - \
				              (s_numPass * s_probPass * s_selfish_scenario)


				# Determina l'azione utilitaristica ideale
				ideal_utilitarian_action = 0 # Dritto
				if util_svolta > util_dritto:
					ideal_utilitarian_action = 1 # Svolta

				# Applica il self-esteem usando l'azione ideale e l'azione della NN
				individual_p.computeSelfEsteem(conf, ideal_utilitarian_action)
		...
		for i, individual_p in enumerate(population):
			...
			if df_scenari_input_for_nn.shape[0] > i:
				...
				# Estrai i valori per il calcolo di 'convieneSvolta' (già fatto sopra per self-esteem, ma replicato per log)
				s_numPed = original_scenario_data["numberOfPedestrians"]
				s_probPed = original_scenario_data["probPed"]
				s_numPass = original_scenario_data["numberOfPassengers"]
				s_probPass = original_scenario_data["probPass"]
				...
				# Ricalcola l'azione utilitaristica ideale per il log (o recuperala se l'hai salvata)
				util_dritto_log = (s_numPass * (1 - s_probPass) * s_selfish_scenario) - \
				                  (s_numPed * s_probPed * s_altruism_scenario)
				util_svolta_log = (s_numPed * (1 - s_probPed) * s_altruism_scenario) - \
				                  (s_numPass * s_probPass * s_selfish_scenario)
				
				conviene_svolta_utilitarian_bool = 0 # Dritto
				if util_svolta_log > util_dritto_log:
					conviene_svolta_utilitarian_bool = 1 # Svolta
				...
				row_data_dict = {
					...
					'convieneSvolta': int(conviene_svolta_utilitarian_bool), # Scelta utilitaristica ideale (0 o 1)
					"predAction": predAction_list_current_gen[i] if i < len(predAction_list_current_gen) else -1 # Azione dell'agente (dalla NN)
				}
		...
		print(f"\tGeneration Metrics: Worst Fitness:{gen_min_fitness_score:.4f} | Avg Fitness:{gen_avg_fitness_score:.4f} | Best Fitness:{gen_max_fitness_score:.4f}")		
